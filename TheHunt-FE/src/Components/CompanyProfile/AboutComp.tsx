		...
		<div className="flex flex-col gap-5">
			{
				Object.keys(company).map((key, index) => key != 'Name' && <div key={index}>
					...
					{/* {key!="Website" && <div className="text-sm text-congress-blue-300 text-justify">{key!="Specialities"?company[key]:company[key].map((item:string, index:number)=><span key={index}> &bull; {item}</span>)}</div>} */}
